...
# orig = orig.loc[:, attributes0 + ['clinsig']]
# orig.columns = [HUMAN_READABLE[i] if i != "clinsig" else "y" for i in orig.columns]
...
# %%
shap.plots.beeswarm(shap_values, max_display=None, #color=plt.get_cmap("RdYlGn_r"), 
                    show=False, plot_size=(12, 12), )

plt.title(f'copy number {cnv_type}')
plt.tight_layout()
# plt.savefig('../aaaaaaaaaaaaaaaaa.png', dpi=200)
...
sns.stripplot(x=temp.value, y=temp.variable, hue=temp.y, size=3, ax=ax,
              palette={'Pathogenic': 'red', 'Benign': 'green'}, zorder=0, alpha=0.5)
...
# plt.savefig(f'plots/results_shap_strip_discrete_{cnv_type}.png')

# %% swarm plot with continuous hue
fig, ax = plt.subplots(1, 1, figsize=(12, 14))
