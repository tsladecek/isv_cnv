...
for cnv_type in ['loss', 'gain']:
    ...
    for i, lt in enumerate(['', '_log']):
        ...
        for model in models:
            if model == 'randomforest':
                results = bar_update_results(results, f'results/models{lt}/{model}_{cnv_type}{lt}.json.gz', f'data/validation_{cnv_type}.tsv.gz', threshold)
            else:
                results = bar_update_results(results, f'results/models{lt}/{model}_{cnv_type}{lt}.json', f'data/validation_{cnv_type}.tsv.gz', threshold)
    ...
    fig.savefig(f'plots/model_bars_{cnv_type}.jpg')
